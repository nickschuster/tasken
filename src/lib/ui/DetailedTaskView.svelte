<script lang="ts">
  ...
  const handleRepeatPreset = (value: string) => {
    ...
    if (value === 'Never') {
      ...
      selectedTask.repeatInterval = null;
      ...
      if (selectedTask.repeatTime == null && preset.unit !== RepeatUnit.HOUR) {
        const defaultRepeatTime = new Time(9, 0);
        selectedTask.repeatTime = defaultRepeatTime.toString();
      }
    }
  ...
  const handleCustomWeekdays = (days: string[]) => {
    ...
    const indices = days
      .map((day) => REPEAT_DAYS.findIndex((d) => d === day))
      .filter((index) => index !== -1);

    selectedTask.repeatDays = indices.length ? indices.join(',') : null;
...
{#if innerWidth !== 0}
  <div
    class={[
      'flex flex-col border-l border-neutral-200 bg-white text-neutral-800 dark:border-neutral-800 dark:bg-neutral-950 dark:text-neutral-200',
      !isMobile && 'relative h-full transition-all duration-150',
      !isMobile && (isTaskViewOpen ? 'w-80' : 'w-0 overflow-hidden'),
      isMobile &&
        'fixed top-0 right-0 z-50 h-screen w-80 shadow-lg transition-transform duration-150',
        ...
        <div class="flex flex-col gap-5">
          ...
          <div class="space-y-2">
            ...
            <div class="flex items-center gap-2">
              <input
                id="task-name"
                type="text"
                class="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-neutral-900 transition placeholder:text-neutral-400 hover:border-neutral-400 focus:border-transparent focus:ring-2 focus:ring-black focus:outline-none md:text-sm dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100 dark:placeholder:text-neutral-500 dark:focus:ring-white"
                bind:value={content}
          ...
          {#if dueDate}
            <div class="space-y-2">
              ...
              >
                <Select.Trigger
                  class="inline-flex w-full items-center justify-between rounded-md border border-neutral-300 bg-white px-3 py-2 text-neutral-900 transition select-none hover:border-neutral-400 focus:border-transparent focus:ring-2 focus:ring-black focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:border-neutral-300 md:text-sm dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100 dark:focus:ring-white dark:disabled:hover:border-neutral-700"
                    ...
                    >
                      <Select.Viewport class="p-1">
                        {#each repeatLabels as label (label.value)}
                          <Select.Item
                            class="flex w-full cursor-pointer items-center gap-2 rounded-md px-3 py-2 text-neutral-800 transition-colors outline-none hover:bg-neutral-100 focus:bg-neutral-100 md:text-sm dark:text-neutral-200 dark:hover:bg-neutral-800 dark:focus:bg-neutral-800"
                            value={label.value}
                            label={label.label}
                          >
            ...
            {#if repeat === 'Custom'}
                ...
                <div class="flex w-full flex-row items-center justify-between">
                  ...
                  <div>
                    <input
                      ...
                      class="w-16 rounded-md border border-neutral-300 bg-white px-3 py-2 text-center text-base text-neutral-900 transition
                    ...
                    >
                      <Select.Trigger
                        class="inline-flex min-w-[96px] items-center justify-between rounded-md
												border border-neutral-300 bg-white px-3 py-2 text-base text-neutral-900
												transition select-none
												hover:border-neutral-400
												focus:border-transparent focus:ring-2 focus:ring-black focus:outline-none
												disabled:cursor-not-allowed disabled:opacity-50
												disabled:hover:border-neutral-300 md:text-sm
												dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100
                              ...
                              {#each repeatUnits as label (label.value)}
                                <Select.Item
                                  class="flex w-full cursor-pointer items-center gap-2 rounded-md
																	px-3 py-2 text-neutral-800 transition-colors outline-none
																	hover:bg-neutral-100
																	focus:bg-neutral-100 md:text-sm
              ...
              {#if customRepeatUnit === RepeatUnit.WEEK}
                <ToggleGroup.Root
                  ...
                  type="multiple"
                  class="flex h-12 items-center gap-x-0.5 rounded-md border border-neutral-300 bg-white px-1 py-1 dark:border-neutral-700 dark:bg-neutral-900"
                >
